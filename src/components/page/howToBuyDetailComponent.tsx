...
function HowToBuyDetailComponent(props: propsContent) {
  ...
  const serviceCarouselImage = props.data.entry.serviceCarouselImageCollection.items;
  ...
  return (
      <section className="w-full h-900px relative mobile:h-auto overflow-hidden">
        ...
        <div className={`w-[50%] pad:w-[70%] mobile:w-auto  pb-67px mobile:pt-67px mx-auto   opacity-1 `}>
          <div className="text-33px font-normal font-AlbertusNova-Regular pb-64px text-center pt-143px pad:pt-102px mobile:pt-42px mobile:text-24px mobile:leading-[40px] mobile:pb-45px uppercase">{title}</div>
          <div className="font-normal font-Grotesque-Regular text-22px text-center text-dark-grey leading-[33px] mobile:text-14px mobile:pl-25px mobile:pr-25px mobile:leading-[20px] flex justify-center">{des}</div>
        </div>
        ...
        <div className="relative overflow-hidden pb-33px mobile:pb-20px" ref={emblaRef}>
          ...
          <div className="flex">
            ...
            {
              serviceCarouselImage.length > 0 && serviceCarouselImage.map((item:carouselImage)=>{
                return (
                    <div key={item.sys.id} className="flex-grow-0 flex-shrink-0 basis-4/6 ml-21px relative h-417px mobile:ml-10px mobile:basis-2/3 mobile:h-455px">
                      <BaseImage
          ...
          <div className="mt-25px bottom-83px w-full flex items-center justify-center mobile:bottom-50px mobile:scale-50">
            {serviceCarouselImage.length > 0 && serviceCarouselImage.map((item, index) => {
              return (
                  <div
                      key={item.sys.id}
