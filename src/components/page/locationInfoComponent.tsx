...
function LocationStoreList(props: any) {
  ...
  return (
    <section className="pt-138px  pad:pt-98px   mobile:pt-112px">
      <div
        className="pb-45px relative z-20 mobile:pb-35px"
        onClick={(e) => {
          if (open) {
            setOpen(false);
          } else {
      ...
      >
        {selectedReady ? (
          <Select
            ...
            options={options}
            onChange={(
              e: SingleValue<{ value: string; label: string } | null>
            ) => {
              setOpen(false);
      ...
      {locationInfo.length > 3 ? (
        <div className="flex justify-end pr-10 pt-97px pad:pt-69px">
          <div>
            <span
              className={`cursor-pointer bg-contain ${
                currentIndex == 0
                  ? "bg-[url('/assets/nearYou/prev.png')]"
                  : "bg-[url('/assets/nearYou/prev-active.png')]"
              } w-44px h-44px inline-block align-middle mr-7px pad:w-31px pad:h-31px pad:mr-5px mobile:w-26px mobile:h-26px`}
              onClick={() => {
                scrollPrev();
              }}
            ></span>
            <span
              className={`cursor-pointer bg-contain ${
                currentIndex == locationInfo.length - 1
                  ? "bg-[url('/assets/nearYou/next.png')]"
                  : "bg-[url('/assets/nearYou/next-active.png')]"
              } w-44px h-44px inline-block align-middle pad:w-31px pad:h-31px mobile:w-26px mobile:h-26px`}
              onClick={() => {
                scrollNext();
              }}
            ></span>
...
function LocationInfoComponent(props: propsContent) {
  ...
  const bottomData = [locationInfoRegionData[3], locationInfoRegionData[4]];
  ...
  return (
    <section
      id="RegionSelect"
      data-anchor={0}
      className="w-full h-screen overflow-hidden bg-cover bg-[url('/assets/howToBuy/bg.png')] bg-[#E6E7E8] relative select-none"
    >
      <input type="hidden" value={headStyle} data-style="headStyle" />

      {isChooseMarquee == false ? (
        <div>
          <div className="pt-104px uppercase font-AlbertusNova-Regular font-normal text-33px text-center pad:pt-110px pad:text-23px mobile:pt-112px mobile:text-24px">
            {title}
          </div>
          <div className=" ">
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          className={`relative inline-block align-middle w-152px h-152px mr-20px  mobile:mr-12px  mobile:w-91px mobile:h-91px `}
                        >
                          <BaseImage
                        ...
                        <div className="text-black inline-block align-bottom font-AlbertusNova-Regular items-center w-291px  mobile:w-175px">
                          {/*pad:text-23px*/}
                          <div className="leading-tight uppercase text-33px mobile:text-20px whitespace-nowrap">
                            {item.name}
                          </div>
                          {/*pad:mt-11px*/}
