...
function StoryChapterOneComponent(props: propsContent) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center absolute left-1/2 -translate-x-1/2 bottom-0 h-[calc(100%_-_422px)] w-full">
        <div className="flex justify-center w-full mobile:top-135px">
          <div
            className={`overflow-hidden text-white font-Grotesque-Regular text-center text-28px transition-all ease-in-out duration-500 delay-1000 h-0 ${
              isCurrentPage ? "h-[6em] mobile:h-90px" : "h-0"
            } w-1162px pad:text-20px pad:w-635px mobile:text-14px mobile:w-275px`}
          >
            <div>{ data.storyChapterOneComponentTextContent }</div>
          </div>
        </div>
        <div
          className={`flex flex-col items-center flex-1 z-10 h-1/2 transition-all ease-in-out duration-500 delay-1000 ${
            isCurrentPage ? "opacity-100" : "opacity-0"
          } mobile:top-244px mobile:object-left-top`}
        >
          <div className="shrink-0 w-78px h-78px border-white border border-solid rounded-full flex items-center justify-center z-10 mobile:w-39px mobile:h-39px">
            <span className="inline-block w-12px h-12px rounded-full bg-white mobile:w-6px mobile:h-6px"></span>
          </div>
          <div className="bg-[0_30px] bg-repeat-y bg-contain w-4px top-38px bg-[url('/assets/story/brand_story_chatper_01_line.png')] mobile:w-2px mobile:top-19px h-screen"></div>
        </div>
      </div>
      <div
